...
{ sort_lib_cpp.count(__LINE__, "quick_sort");
    ...
    if (sort_lib_cpp.conditionCount(__LINE__) && data.size() == 0)
        { return; }
...
{ sort_lib_cpp.count(__LINE__, "quick_sort");
      ...
      int idx = left;
      int jdx = right;
      while (sort_lib_cpp.conditionCount(__LINE__) && idx <= jdx) {
            while (sort_lib_cpp.conditionCount(__LINE__) && LESS_THAN(data[Vec_Idx(idx)], pivot)) {
                idx++; sort_lib_cpp.count(__LINE__);
            }
            while (sort_lib_cpp.conditionCount(__LINE__) && GREATER_THAN(data[Vec_Idx(jdx)], pivot)) {
                jdx--; sort_lib_cpp.count(__LINE__);
            }

            if (sort_lib_cpp.conditionCount(__LINE__) && idx <= jdx)
            {
      ...
      // Recurse
      if (sort_lib_cpp.conditionCount(__LINE__) && left < jdx)
            { quick_sort(data, left, jdx); sort_lib_cpp.count(__LINE__); }

      if (sort_lib_cpp.conditionCount(__LINE__) && idx < right)
...
{ sort_lib_cpp.count(__LINE__, "selection_sort");
    ...
    // Do the sort
    while (sort_lib_cpp.conditionCount(__LINE__) && last > 0)
    {
        // Find greatest in unsorted part
        Vec_Idx idx_of_greatest = 0;
        for (Vec_Idx idx = 0; sort_lib_cpp.conditionCount(__LINE__) && idx <= last; ++idx)
        {
            if ( sort_lib_cpp.conditionCount(__LINE__) && LESS_THAN(data[idx_of_greatest], data[idx]) )
            {
...
{ sort_lib_cpp.count(__LINE__, "bubble_sort");
    ...
    for(Vec_Idx limit = data.size(); sort_lib_cpp.conditionCount(__LINE__) && limit > 0; limit--)
    {
        // Go through vector once, swap element and next element if out of order
        for(Vec_Idx idx = 0; sort_lib_cpp.conditionCount(__LINE__) && idx < limit - 1; idx++)
        {
            if( sort_lib_cpp.conditionCount(__LINE__) && LESS_THAN(data[idx + 1], data[idx]) )
            {
