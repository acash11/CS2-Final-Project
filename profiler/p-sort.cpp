...
{ sort_cpp.count(__LINE__, "main");
    ...
    if(sort_cpp.conditionCount(__LINE__) && opts._output_data)
    ...
    if (sort_cpp.conditionCount(__LINE__) && opts._quick_sort)     { quick_sort(data); sort_cpp.count(__LINE__);      }
    if (sort_cpp.conditionCount(__LINE__) && opts._selection_sort) { selection_sort(data); sort_cpp.count(__LINE__);  }
    if (sort_cpp.conditionCount(__LINE__) && opts._bubble_sort)    { bubble_sort(data); sort_cpp.count(__LINE__);     }
    if ( sort_cpp.conditionCount(__LINE__) && !opts._quick_sort      &&
         !opts._selection_sort  &&
         !opts._bubble_sort  )
        { output_error_and_exit("No sort specified."); sort_cpp.count(__LINE__); }

    // Output data after sorting
    if(sort_cpp.conditionCount(__LINE__) && opts._output_sorted_data)
...
{ sort_cpp.count(__LINE__, "generate_random_data");
    ...
    for (vector<int>::size_type idx = 0; sort_cpp.conditionCount(__LINE__) && idx < vec.size(); ++idx)
    {
        if(sort_cpp.conditionCount(__LINE__) && mod) { vec[idx] = random() % mod; sort_cpp.count(__LINE__); }
...
{ sort_cpp.count(__LINE__, "output_data");
    ...
    for (vector<int>::size_type idx = 0; sort_cpp.conditionCount(__LINE__) && idx < vec.size(); ++idx)
    {
        ...
        if ( sort_cpp.conditionCount(__LINE__) && ! (idx % cols) )
...
{ sort_cpp.count(__LINE__, "process_command_line");
    ...
    if (sort_cpp.conditionCount(__LINE__) && argc == 1)
        { output_usage_and_exit(argv[0]); sort_cpp.count(__LINE__); }

    // Go through the argumets
    for (int idx = 1; sort_cpp.conditionCount(__LINE__) && idx < argc; ++idx)
    {
        // Standard library string from C-string
        string opt(argv[idx]);

        // Process the option
        if (sort_cpp.conditionCount(__LINE__) && opt == "-h")   { output_usage_and_exit(argv[0]); sort_cpp.count(__LINE__); }
        if (sort_cpp.conditionCount(__LINE__) && opt == "-qs")  { opts._quick_sort         = true; sort_cpp.count(__LINE__);  }
        if (sort_cpp.conditionCount(__LINE__) && opt == "-ss")  { opts._selection_sort     = true; sort_cpp.count(__LINE__);  }
        if (sort_cpp.conditionCount(__LINE__) && opt == "-bs")  { opts._bubble_sort        = true; sort_cpp.count(__LINE__);  }
        if (sort_cpp.conditionCount(__LINE__) && opt == "-od")  { opts._output_data        = true; sort_cpp.count(__LINE__);  }
        if (sort_cpp.conditionCount(__LINE__) && opt == "-osd") { opts._output_sorted_data = true; sort_cpp.count(__LINE__);  }
        if (sort_cpp.conditionCount(__LINE__) && opt == "-sz")
        {
            if (sort_cpp.conditionCount(__LINE__) && idx + 1 < argc) { ++idx; sort_cpp.count(__LINE__); opts._data_size = atoi(argv[idx]); sort_cpp.count(__LINE__); }
            else                { output_error_and_exit("Value for -sz option is missing."); sort_cpp.count(__LINE__); }
        }
        if (sort_cpp.conditionCount(__LINE__) && opt == "-rs")
        {
            if (sort_cpp.conditionCount(__LINE__) && idx + 1 < argc) { ++idx; sort_cpp.count(__LINE__); opts._seed = atoi(argv[idx]); sort_cpp.count(__LINE__); }
            else                { output_error_and_exit("Value for -rs option is missing."); sort_cpp.count(__LINE__); }
        }
        if (sort_cpp.conditionCount(__LINE__) && opt == "-mod")
        {
            if (sort_cpp.conditionCount(__LINE__) && idx + 1 < argc) { ++idx; sort_cpp.count(__LINE__); opts._mod = atoi(argv[idx]); sort_cpp.count(__LINE__); }
            else                { output_error_and_exit("Value for -mod option is missing."); sort_cpp.count(__LINE__); }
        }
        if ( sort_cpp.conditionCount(__LINE__) && (opt != "-h")   &&
             (opt != "-qs")  &&
             (opt != "-ss")  &&
             (opt != "-bs")  &&
             (opt != "-od")  &&
